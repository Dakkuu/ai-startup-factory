...
START=pd.Timestamp('2015-01-01'); END=pd.Timestamp('2026-07-29')
...
m=m[m.code.str.match(STOCK_RE)].copy(); m=m[(m.end>=START)&(m.start<=END)]
codes=sorted(m.code.unique())
# BaoStock does not reliably cover Beijing-board securities; retain them in manifest but fetcher will fail closed per code.
(OUT/'codes.txt').write_text('\n'.join(codes),encoding='utf-8')
sig=cal[(cal>=START)&(cal<=END)][::5]
(OUT/'signal_dates.txt').write_text('\n'.join(pd.Series(sig).dt.strftime('%Y-%m-%d')),encoding='utf-8')
pd.DataFrame([{'release_tag':RELEASE_TAG,'start':str(START.date()),'end':str(END.date()),'codes':len(codes),'signal_dates':len(sig),'min_member_start':str(m.start.min().date()),'max_member_end':str(m.end.max().date())}]).to_csv(OUT/'manifest.csv',index=False)
