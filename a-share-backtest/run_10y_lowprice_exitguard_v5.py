...
OUT=Path('results_lowprice_exitguard_v5'); OUT.mkdir(exist_ok=True)
...
# Five predeclared stock-level guards. Thresholds are simple economic break points,
# not optimized on annual outcomes.
GUARDS=('none','rmom_positive','tstat_positive','rmom_and_tstat_positive','dd_above_minus25')
...
def guarded(q,name):
    ...
    if name=='rmom_positive': ok=np.isfinite(x.rmom126)&(x.rmom126>0)
    ...
    elif name=='dd_above_minus25': ok=np.isfinite(x.dd120)&(x.dd120>-0.25)
    else: raise ValueError(name)
    # Guard failures are ineligible on T. This both prevents entry and forces an
    # incumbent out at the later execution open when tradable; no future data used.
    x.loc[m & ~ok,'rank_test']=np.nan
    return x
...
def main():
    ...
    baseq=lp.rank_signal(p,WEIGHTS,LIQ,FLOOR); rows=[]
    for name in GUARDS:
        q=guarded(baseq,name); eq,sts,tms=eval_all(q,cal,members,bm,START,TRAIN_END,1e6,1.); r=robust(eq,sts); r['guard']=name; rows.append(r); print('TRAIN',name,r,flush=True)
    t=pd.DataFrame(rows); t.to_csv(OUT/'train_guards.csv',index=False)
    ...
    q=guarded(baseq,str(win['guard'])); eq,sts,tms=eval_all(q,cal,members,bm,START,END,1e6,1.)
    full=fa.perf_eq(eq,bm); full['train_2016_2021_return']=fa.period_return(eq,START,TRAIN_END); full['pseudo_oos_2022_2026_return']=fa.period_return(eq,PSEUDO,END); full.update(guard=win['guard'],hold=HOLD,n_hold=N,entry=ENTRY,keep=KEEP,selection_validation_accesses=0)
    ...
    pd.DataFrame([{**ua,'market_factor':market_code,'experiment':'LowPrice stock-level exit guard V5','guards':'|'.join(GUARDS),'candidate_count':len(GUARDS),'selection_period':'2016-07-29..2021-12-31','validation_2022_2026_accesses_before_freeze':0,'construction':'frozen H60/N4/Entry15/Keep40','guard_timing':'T-only rmom126/tstat120/dd120; execution later open','selection':'all12 phase exact split; both train halves positive; MDD>-50%; robust score','executor':'hard_v3; board limits; 100-share lots; no replacement'}]).to_csv(OUT/'audit.csv',index=False)
