...
def main():
    ...
    # Keep all execution/price/volume fields at original float64 precision because lot rounding
    # can change trades if raw prices/factors are downcast. Only percentile ranks are compacted.
    for c in [x for x in s.columns if x.startswith('rank_')]:
    ...
    meta={'status':'REUSABLE_SIGNAL_PANEL_FOR_PREREG_MULTI_ALPHA_V2','rows':len(s),'signal_dates':int(s.signal_date.nunique()),'market_factor':market_code,'release_tag':'2026-07-29','prereg':'MULTI_ALPHA_PREREG_2026-09-03.md','rank_columns':[c for c in s if c.startswith('rank_')],'execution_fields_float64_preserved':True,'universe_audit':ua}
