...
OUT=Path('results_lowprice_entryguard_v5'); OUT.mkdir(exist_ok=True)
START=mo.START; TRAIN_END=mo.TRAIN_END; PSEUDO=mo.PSEUDO_START; END=mo.END
WEIGHTS={'price':.35,'iv':.18,'ef':.15,'rmom':.20,'tstat':.12}
LIQ=.55; FLOOR=2.; HOLD=60; N=4; ENTRY=.15; KEEP=.40; NPHASE=12
GUARDS=('none','rmom_positive','tstat_positive','rmom_and_tstat_positive','mom120_positive')
BASECOLS=strict.BASECOLS+['exec_buy_allowed','exec_sell_allowed','exec_open_gap','exec_limit_proxy','signal_entry_allowed']

# Keep logic is unchanged for incumbents. The extra flag affects entrants only.
def choose_entry_guard(g,current):
    col='ivol60_pct'
    ...
    keep=keep[:hard.N_HOLD]
    if len(keep)<hard.N_HOLD:
        e=x[(x[col]<=sim.ENTRY_PCT)&x.signal_entry_allowed.astype(bool)]
        entrants=[c for c in e.code.tolist() if c not in current and c not in keep]
        keep.extend(entrants[:hard.N_HOLD-len(keep)])
    return keep[:hard.N_HOLD]
hard.choose_det=choose_entry_guard


def with_guard(q,name):
    x=q.copy(); ok=pd.Series(True,index=x.index)
    if name=='none': pass
    elif name=='rmom_positive': ok=np.isfinite(x.rmom126)&(x.rmom126>0)
    elif name=='tstat_positive': ok=np.isfinite(x.tstat120)&(x.tstat120>0)
    elif name=='rmom_and_tstat_positive': ok=np.isfinite(x.rmom126)&np.isfinite(x.tstat120)&(x.rmom126>0)&(x.tstat120>0)
    elif name=='mom120_positive': ok=np.isfinite(x.mom120raw)&(x.mom120raw>0)
    else: raise ValueError(name)
    x['signal_entry_allowed']=np.asarray(ok,bool)
    return x
...
def eval_all(q,cal,members,bm,start,end,total_cash=1e6,cost=1.):
    ...
    return combine_abs(eqs,[per]*NPHASE,start),sts,tms
def robust(eq,sts):
    ...
    hardpass=int((rr>0).all() and h1>0 and h2>0 and em>-0.50)
    score=float(ec+.50*np.min(cg)-.25*np.std(cg)+.25*min(h1,h2))
    return {'train_return':eq_return(eq),'train_cagr':ec,'train_mdd':em,'min_phase_return':rr.min(),'median_phase_return':np.median(rr),'min_phase_cagr':cg.min(),'median_phase_cagr':np.median(cg),'std_phase_cagr':cg.std(),'half1_cagr':h1,'half2_cagr':h2,'all_phases_positive':int((rr>0).all()),'hard_pass':hardpass,'robust_score':score}

def main():
    p,cal,members,ua,market_code,bm=mo.build_panel(OUT,need_fwd=False); p=lp.attach_price(p,cal); p=strict.attach_gap_flags(p,cal,'board'); baseq=lp.rank_signal(p,WEIGHTS,LIQ,FLOOR)
    rows=[]
    for name in GUARDS:
        q=with_guard(baseq,name); eq,sts,tms=eval_all(q,cal,members,bm,START,TRAIN_END,1e6,1.); r=robust(eq,sts); r['entry_guard']=name; rows.append(r); print('TRAIN',name,r,flush=True)
    t=pd.DataFrame(rows); t.to_csv(OUT/'train_entry_guards.csv',index=False)
    ...
    q=with_guard(baseq,str(win['entry_guard'])); eq,sts,tms=eval_all(q,cal,members,bm,START,END,1e6,1.)
    full=fa.perf_eq(eq,bm); full['train_2016_2021_return']=fa.period_return(eq,START,TRAIN_END); full['pseudo_oos_2022_2026_return']=fa.period_return(eq,PSEUDO,END); full.update(entry_guard=win['entry_guard'],hold=HOLD,n_hold=N,entry=ENTRY,keep=KEEP,selection_validation_accesses=0)
    ...
    pd.DataFrame([{**ua,'market_factor':market_code,'experiment':'LowPrice entry-only guard V5','guards':'|'.join(GUARDS),'candidate_count':len(GUARDS),'selection_period':'2016-07-29..2021-12-31','validation_2022_2026_accesses_before_freeze':0,'construction':'frozen H60/N4/Entry15/Keep40','entry_guard_semantics':'applies only to new entrants; incumbents keep standard rank<=40% hysteresis','timing':'all guard fields known at T close; execution later open','selection':'all12 phase exact split; both train halves positive; MDD>-50%; robust score','executor':'hard_v3 with patched deterministic chooser; board limits; 100-share lots; no replacement'}]).to_csv(OUT/'audit.csv',index=False)
