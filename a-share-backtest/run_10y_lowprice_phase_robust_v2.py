...
def main(shard):
    ...
    for vname in SHARDS[shard]:
      ...
      for liq in LIQS:
       for floor in FLOORS:
        print('SIGNAL',shard,vname,liq,floor,flush=True); q=lp.rank_signal(p,w,liq,floor)
        for h in HOLDS:
         phs=screen_phases(h)
         for n in NS:
          for e,k in BUFFERS:
            vals=[]
            for ph in phs:
                st,eq,tr,tm=run_one(q,h,ph,n,e,k,cal,members,bm,1e6); vals.append(st)
            rets=np.array([x['total_return'] for x in vals],float); cgs=np.array([x['cagr'] for x in vals],float); mdds=np.array([x['max_drawdown'] for x in vals],float)
    ...
    finals=[]
    # Rebuild one signal at a time for finalists; no multi-panel cache.
    for (vname,liq,floor),grp in top.groupby(['variant','liq','floor'],sort=False):
        q=lp.rank_signal(p,VARIANTS[str(vname)],float(liq),float(floor))
        for r in grp.itertuples(index=False):
            pc=phase_count(r.hold); per=1e6/pc; eqs=[]; phase_cg=[]; phase_ret=[]; phase_mdd=[]
            for ph in range(pc):
