...
START=pd.Timestamp('2007-01-04'); END=pd.Timestamp('2016-07-28'); WARM=pd.Timestamp('2005-01-01')
...
def load_base_holdout():
    ...
    t=cal[(cal>=START)&(cal<=END)]; cnt=np.array([int(((m.start<=d)&(m.end>=d)).sum()) for d in t])
    union=int(m[(m.end>=START)&(m.start<=END)].code.nunique()); entered=int(m[(m.start>START)&(m.start<=END)].code.nunique()); exited=int(m[(m.end>=START)&(m.end<END)].code.nunique())
...
def build_minimal_panel(cal,members,market_close,ua):
    # Current strategy only needs liquidity, IVOL60, execution fields; do not let
    # obsolete RMOM research eligibility silently constrain this holdout.
    trade_cal=cal[(cal>=START)&(cal<=END)]; signal_dates=pd.DatetimeIndex(trade_cal[::5]); alltrade=cal[cal<=END]
    exec_dates=[]
    ...
    for i,code in enumerate(codes,1):
        ...
        else: z['factor']=z.factor.replace(0,np.nan).fillna(1.)
    ...
    gs=p.groupby('signal_date').size(); panel_audit={'signal_dates':int(p.signal_date.nunique()),'rows':len(p),'min_eligible':int(gs.min()),'median_eligible':float(gs.median()),'max_eligible':int(gs.max()),'median_over_min_active':float(gs.median()/ua['min_daily_members'])}
    pd.DataFrame([panel_audit]).to_csv(OUT/'panel_audit.csv',index=False)
    gates={'signal_dates_ge_450':p.signal_date.nunique()>=450,'median_eligible_ge_half_min_active':gs.median()>=.5*ua['min_daily_members'],'min_eligible_ge_300':gs.min()>=300}
    pd.DataFrame([{'gate':k,'pass':int(v)} for k,v in gates.items()]).to_csv(OUT/'panel_gates.csv',index=False)
    if not all(gates.values()): raise RuntimeError(f'FAIL-CLOSED holdout panel {panel_audit} {gates}')
    return p
...
def main():
    sim.START=START; sim.END=END; sim.WARM=WARM; base.START=START; base.END=END; base.WARM=WARM; base.OUT=OUT; v4.OUT=OUT
    cal,members,ua=load_base_holdout(); market_code,market_close,_=v4.pick_market(cal); p=build_minimal_panel(cal,members,market_close,ua)
    p=fq.add_factors(p,cal); p=sf.add_skews(p,cal,market_close); bm=market_close.loc[START:END].dropna(); q=be.anchor_weighted(p,'liq70',.60)
    ...
    for name,a,b in [('2007_2009','2007-01-04','2009-12-31'),('2010_2012','2010-01-01','2012-12-31'),('2013_2016','2013-01-01','2016-07-28')]:
        z=period_metrics(eq,a,b); z['block']=name; blocks.append(z)
    bl=pd.DataFrame(blocks); bl.to_csv(OUT/'blocks.csv',index=False)
    costs=[]
    for cm in (2.,4.,8.):
        z=ma.subset_phase(q,60,0); x,_,_,_=ma.run_panel(z,cal,members,bm,n=20,entry=.10,keep=.30,cost=cm); costs.append({**x,'cost_mult_test':cm})
    co=pd.DataFrame(costs); co.to_csv(OUT/'costs.csv',index=False)
    delays=[]
    for d in (1,3,5):
        z=ma.delay_panel(q,d,cal,members); x,_,_,_=ma.run_panel(z,cal,members,bm,n=20,entry=.10,keep=.30); delays.append({**x,'delay_sessions':d})
    ...
    pd.DataFrame([verdict]).to_csv(OUT/'verdict.csv',index=False); print('=== BACKWARD HOLDOUT VERDICT ==='); print(pd.DataFrame([verdict]).to_string(index=False),flush=True); print('=== GATES ==='); print(gd.to_string(index=False),flush=True); print('=== PHASES ==='); print(ph[['phase','total_return','cagr','max_drawdown','sharpe']].to_string(index=False),flush=True); print('=== BLOCKS ==='); print(bl.to_string(index=False),flush=True); print('=== COSTS ==='); print(co[['cost_mult_test','total_return','cagr','max_drawdown','sharpe']].to_string(index=False),flush=True); print('=== DELAYS ==='); print(de[['delay_sessions','total_return','cagr','max_drawdown','sharpe']].to_string(index=False),flush=True)
