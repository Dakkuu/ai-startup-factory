...
MAX_NAMES=5; COMMISSION=0.00025; MIN_COMMISSION=5.0; SLIPPAGE=0.0020; PARTICIPATION=0.02
VARIANTS=["01_leader_only","02_network_leader","03_network_auction","04_network_auction_lhb","05_full_regime_network_auction_lhb_event"]

def pct_rank(x):
    x=np.asarray(x,float); out=np.full(x.shape,np.nan); ok=np.isfinite(x)
    if ok.sum()==0:return out
    vals=x[ok]; order=np.argsort(np.argsort(vals,kind="mergesort"),kind="mergesort"); out[ok]=(order+0.5)/len(vals); return out

def rolling_mean_mat(a,w):
    a=np.asarray(a,float); out=np.full_like(a,np.nan,dtype=float); s=np.nancumsum(np.where(np.isfinite(a),a,0.0),axis=0); c=np.cumsum(np.isfinite(a),axis=0)
    for t in range(w-1,len(a)):
        s0=s[t]-(s[t-w] if t>=w else 0); c0=c[t]-(c[t-w] if t>=w else 0); out[t]=np.divide(s0,c0,out=np.full(a.shape[1],np.nan),where=c0>=max(3,w//2))
    return out
...
def get_limit_pct(code,d):
    ...
    if code.startswith("SH") and s.startswith("688"):return .20
    if code.startswith("SZ") and s.startswith(("300","301")) and pd.Timestamp(d)>=pd.Timestamp("2020-08-24"):return .20
    return .10

def fee(gross,side,d):
    if gross<=0:return 0.0
    f=max(MIN_COMMISSION,gross*COMMISSION)+gross*.00001
    if side=="sell":f+=gross*(.0005 if pd.Timestamp(d)>=pd.Timestamp("2023-08-28") else .001)
    return f
...
def fetch_lhb(start,end):
    ...
    for y in range(start.year,end.year+1):
        ...
        except Exception as e:print("LHB ERROR",y,type(e).__name__,str(e)[:200],flush=True);audit.append((y,0,"error:"+type(e).__name__))
    pd.DataFrame(audit,columns=["year","rows","status"]).to_csv(OUT/"lhb_coverage.csv",index=False);return pd.concat(rows,ignore_index=True) if rows else pd.DataFrame()
...
def fetch_sparse_events(start,end):
    ...
    for y in range(max(2017,start.year),end.year+1):
        ...
        for kw,score in EVENT_QUERIES.items():
            try:
                df=ak.stock_zh_a_disclosure_report_cninfo(symbol="",market="沪深京",keyword=kw,category="",start_date=a.strftime("%Y%m%d"),end_date=b.strftime("%Y%m%d"));n=0 if df is None else len(df);audit.append((y,kw,n,"ok"))
                if n:
                    z=df[["代码","简称","公告标题","公告时间"]].copy();z["code"]=z["代码"].map(map_code);z["announce_time"]=pd.to_datetime(z["公告时间"],errors="coerce");z["date"]=z["announce_time"].dt.normalize();z["event_raw"]=score;z["keyword"]=kw;rows.append(z[["date","code","event_raw","keyword","公告标题","announce_time"]])
                print("EVENT",y,kw,n,flush=True)
            except Exception as e:audit.append((y,kw,0,"error:"+type(e).__name__));print("EVENT ERROR",y,kw,type(e).__name__,str(e)[:200],flush=True)
...
def build_regime(dates,member,ret1,limitup,limitdn,close):
    ...
    for t in range(n):
        ...
        if t>0:p=member[t-1]&limitup[t-1]&np.isfinite(ret1[t]);prem=np.nanmean(ret1[t,p]) if p.any() else np.nan;reseal=np.nanmean(limitup[t,p]) if p.any() else np.nan
        else:prem=reseal=np.nan
        rec.append([dates[t],up,dn,med,br,prem,reseal])
    df=pd.DataFrame(rec,columns=["date","limitup_share","limitdn_share","median_ret","breadth20","board_premium","reseal_rate"]);df["up_med60"]=df.limitup_share.rolling(60,min_periods=20).median();df["up_q85_60"]=df.limitup_share.rolling(60,min_periods=20).quantile(.85);df["dn_q80_60"]=df.limitdn_share.rolling(60,min_periods=20).quantile(.80);states=[]
    for i in range(len(df)):
        def num(col,default=np.nan):
            v=df[col].iat[i]
            try:return float(v) if pd.notna(v) else default
            except Exception:return default
        up=num("limitup_share");dn=num("limitdn_share");med=num("median_ret");br=num("breadth20");prem=num("board_premium");rs=num("reseal_rate");um=num("up_med60",.01);uq=num("up_q85_60",.02);dq=num("dn_q80_60",.005)
        if (np.isfinite(prem) and prem<-.025) or (np.isfinite(dn) and dn>max(.008,dq) and np.isfinite(med) and med<-.01):st="retreat"
        elif np.isfinite(br) and br<.28 and np.isfinite(up) and up<max(.004,um*.6):st="ice"
        elif np.isfinite(up) and up>max(uq,.02) and np.isfinite(prem) and prem>.025 and np.isfinite(br) and br>.6:st="climax"
        elif np.isfinite(prem) and prem>.015 and np.isfinite(rs) and rs>.25 and np.isfinite(br) and br>.45:st="main"
        elif np.isfinite(med) and med>0 and (not np.isfinite(prem) or prem>-.005) and np.isfinite(br) and br>.33:st="repair"
        else:st="divergence"
        states.append(st)
...
@dataclass
class Position:units:int;entry_px:float;entry_date:pd.Timestamp;signal_date:pd.Timestamp;peak:float;entry_value:float;entry_fee:float
...
def compute_daily_candidates(t,dates,codes,member,close,volume,ret1,mom5,volratio,amount_rank,limitup,streak,lhb_map,event_map):
    ...
    ids=np.where(valid)[0];att=.30*pct_rank(ret1[t,ids])+.25*pct_rank(mom5[t,ids])+.20*pct_rank(np.log1p(np.maximum(volratio[t,ids],0)))+.15*pct_rank(amount_rank[t,ids])+.10*np.minimum(streak[t,ids],3)/3;take=ids[np.argsort(np.nan_to_num(att,nan=-9))[-80:]];k=len(take);lo=max(1,t-14);H=ret1[lo:t+1,take].copy()
    for r in range(H.shape[0]):mm=member[lo+r]&np.isfinite(ret1[lo+r]);H[r]-=np.nanmedian(ret1[lo+r,mm]) if mm.any() else 0
    H=np.nan_to_num(H,nan=0,posinf=0,neginf=0);H-=H.mean(axis=0,keepdims=True);sd=H.std(axis=0,keepdims=True);sd[sd<1e-6]=1;Z=H/sd;corr=(Z.T@Z)/max(1,H.shape[0]-1);np.fill_diagonal(corr,0);adj=corr>.55;pc=adj.sum(axis=1);tr=np.nan_to_num(ret1[t,take],nan=0);tl=limitup[t,take].astype(float);tv=np.nan_to_num(np.log1p(np.maximum(volratio[t,take],0)),nan=0);peer_strength=np.divide(adj@tr,pc,out=np.zeros(k),where=pc>0);peer_limit=np.divide(adj@tl,pc,out=np.zeros(k),where=pc>0);peer_vol=np.divide(adj@tv,pc,out=np.zeros(k),where=pc>0);theme=pct_rank(1.8*peer_strength+1.2*peer_limit+.15*peer_vol+.015*np.minimum(pc,20));lead_raw=1.2*np.nan_to_num(mom5[t,take])+.7*np.nan_to_num(ret1[t,take])+.12*np.minimum(streak[t,take],4)+.18*np.log1p(np.maximum(volratio[t,take],0))+.35*np.nan_to_num(theme);leader=pct_rank(lead_raw);capacity=pct_rank(.55*pct_rank(amount_rank[t,take])+.25*leader+.20*theme);crowd=(np.maximum(mom5[t,take]-.28,0)/.28+(streak[t,take]>=3)*.7+np.maximum(volratio[t,take]-5,0)/5+(close[t,take]<5)*.25);crowd=np.clip(crowd,0,3)/3;base_score=.35*pct_rank(mom5[t,take])+.20*pct_rank(ret1[t,take])+.20*pct_rank(volratio[t,take])+.15*pct_rank(amount_rank[t,take])+.10*np.minimum(streak[t,take],3)/3;d=pd.Timestamp(dates[t]).normalize();rows=[]
    for j,idx in enumerate(take):
        l=lhb_map.get((d,codes[idx]));lhb=0 if l is None else (0 if not np.isfinite(l.get("net_ratio",np.nan)) else np.tanh(l["net_ratio"]/10.0))+(0.25 if np.isfinite(l.get("net",np.nan)) and l["net"]>0 else -0.15);ev=event_map.get((d,codes[idx]),0.0);ev=ev*max(.20,1-max(mom5[t,idx],0)/.30) if ev>0 and np.isfinite(mom5[t,idx]) else ev;rows.append({"idx":idx,"code":codes[idx],"base":float(base_score[j]),"theme":float(theme[j]),"leader":float(leader[j]),"capacity":float(capacity[j]),"crowd":float(crowd[j]),"lhb":float(lhb),"event":float(ev),"peer_count":int(pc[j]),"streak":int(streak[t,idx]),"ret1":float(ret1[t,idx]),"mom5":float(mom5[t,idx]),"volratio":float(volratio[t,idx])})
...
def stats(eq,trades):
    ...
    if len(t):wins=t.net_pnl[t.net_pnl>0];losses=t.net_pnl[t.net_pnl<0];pf=wins.sum()/(-losses.sum()) if losses.sum()<0 else np.nan;wr=(t.net_pnl>0).mean()
    else:pf=wr=np.nan
...
def main():
    dates,codes,close,open_,high,volume,factor,member,load_audit=base.load_data();dates=pd.DatetimeIndex(dates);codes=list(codes);member=member.astype(bool);stock_mask=np.array([bool(base.STOCK_RE.match(c)) for c in codes],dtype=bool);member[:,~stock_mask]=False;n,p=close.shape;print("DATA",n,p,"stock member union",int(member.any(axis=0).sum()),flush=True)
    ...
    for t in range(n):ids=np.where(member[t]&np.isfinite(liq[t]))[0];amount_rank[t,ids]=pct_rank(liq[t,ids])
    limitup=np.zeros((n,p),bool);limitdn=np.zeros((n,p),bool)
    for t in range(1,n):
        for idx in np.where(member[t]&np.isfinite(ret1[t]))[0]:lim=get_limit_pct(codes[idx],dates[t]);limitup[t,idx]=ret1[t,idx]>=lim*.985;limitdn[t,idx]=ret1[t,idx]<=-lim*.985
    streak=np.zeros((n,p),np.int16)
    for t in range(1,n):streak[t]=np.where(limitup[t],streak[t-1]+1,0)
    market=build_regime(dates,member,ret1,limitup,limitdn,close);market.to_csv(OUT/"market_regime.csv",index=False);lhb=fetch_lhb(START,END);lhb_map={}
    if len(lhb):
    ...
    for t in range(max(start_i,25),end_i+1):cache[t]=compute_daily_candidates(t,dates,codes,member,close,volume,ret1,mom5,volratio,amount_rank,limitup,streak,lhb_map,event_map);print("candidate day",t,"n",len(cache[t]),flush=True) if t%250==0 else None
    ...
    for variant in VARIANTS:
        ...
        for t in range(start_i,end_i+1):
            ...
            eq.append({"strategy":variant,"date":d,"equity":cash+holdings,"cash":cash,"positions":len(pos)});state=market.iloc[t].state
            ...
            if not ranked.empty:
                ...
                if variant.startswith("05_"):ranked=ranked.iloc[0:0] if state=="retreat" else ranked[ranked.score>=.42]
