...
def fund_ranks(p,va,sa):
    ...
    # Reproduce the prior fundamental factor definitions exactly: robust cross-sectional z first,
    # then percentile rank (lower rank = more desirable).
    ...
    rawmap={'book_yield':'book_yield_z','value3':'value3_raw','quality_value':'quality_value_raw',
            'cfo_assets':'cfo_assets_z','accrual_quality':'accrual_quality_z','cash_conversion':'cash_conversion_z'}
...
def blend(q, weights):
    # weights keys: tech + fundamental rank column names; missing fundamentals are neutral (0.5),
    # matching the earlier single-factor blend convention.
...
def run_all(cands,cal,members,bm,cost=1.0,only=None):
    rows=[]
    names=list(cands) if only is None else list(only)
    for ci,nm in enumerate(names,1):
        ...
        for h in HORIZONS:
            ...
            for ph in range(step):
                ...
                st.update(candidate=nm,H=h,phase=ph,cost_mult=cost,
                          half1_cagr=pc(eq,START,pd.Timestamp('2019-12-31')),
                          half2_cagr=pc(eq,pd.Timestamp('2020-01-01'),TRAIN_END),
                          pseudo_cagr=pc(eq,PSEUDO,END))
                rows.append(st)
...
def summarize(d):
    ...
    for c,g in d.groupby('candidate'):
        rows.append({'candidate':c,'runs':len(g),
          'full_cagr_median':g.cagr.median(),'full_cagr_p25':g.cagr.quantile(.25),'full_cagr_min':g.cagr.min(),
          'mdd_median':g.max_drawdown.median(),'mdd_worst':g.max_drawdown.min(),'sharpe_median':g.sharpe.median(),
          'train_maximin':min(g.half1_cagr.median(),g.half2_cagr.median()),
          'pseudo_cagr_median':g.pseudo_cagr.median(),'pseudo_cagr_p25':g.pseudo_cagr.quantile(.25),
          'h60_median':g[g.H==60].cagr.median(),'h75_median':g[g.H==75].cagr.median(),'h90_median':g[g.H==90].cagr.median()})
...
def main():
    ...
    vpath=locate('artifact_cache/value/**/pit_value_attached.csv.gz')
    spath=locate('artifact_cache/stmt/**/pit_attached.csv.gz')
    ...
    # Predeclared robustness stress: stress the three highest P25 non-baseline candidates plus baselines.
    ...
    meta={'status':'NEW_STOCK_LEVEL_PIT_INTEGRATED_BACKTEST_NOT_ORIGINAL_EXACT',
          'fundamental_inputs':'period-corrected V2 artifacts from run 33634216408',
          'artifact_row_identity_pass':True,'pit_rule':'next exchange day after announcement; stale >550d neutral/missing',
          'horizons':list(HORIZONS),'N':N,'entry':ENTRY,'keep':KEEP,
          'old_mom_weights':BASE_W,'riskcore_weights':RISK_W,
          'control_h90_phase0_cagr':float(control.cagr),
          'original_exact_h90_phase0_cagr':0.30697333,
          'source_identity_pass':False,'market_factor':market_code,'fundamental_rank_coverage':cov,
          'universe_audit':ua,'cost2x_finalists':finalists}
