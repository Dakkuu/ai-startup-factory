...
def build_panel(cal, members, market_close):
    ...
    signal_dates=pd.DatetimeIndex(trade_cal[::5]); alltrade=cal[cal<=a.END]; exec_dates=[]
    ...
    frames=[]; codes=sorted(members.code.unique()); rows_with_missing_exec=0
    for i,code in enumerate(codes,1):
        ...
        r=z.close.pct_change(fill_method=None); count120=z.close.notna().rolling(a.MIN_LIST_DAYS).sum(); liq20=(z.close.abs()*z.volume.abs()).rolling(20).mean()
        m=bm_ret.reindex(z.index); smu=r.rolling(a.BETA_LOOKBACK,min_periods=a.MIN_BETA_OBS).mean().shift(1); cov=r.rolling(a.BETA_LOOKBACK,min_periods=a.MIN_BETA_OBS).cov(m).shift(1)
        beta=cov/bm_var.reindex(z.index); alpha=smu-beta*bm_mu.reindex(z.index); resid=r-alpha-beta*m
        fac={}
        for w in a.RMOM_WINDOWS:
            n=w-a.SKIP_RECENT+1; fac[f'rmom{w}']=resid.shift(a.SKIP_RECENT).rolling(n,min_periods=max(30,int(n*.8))).sum()
        for w in a.IVOL_WINDOWS: fac[f'ivol{w}']=resid.rolling(w,min_periods=max(25,int(w*.8))).std()
        ...
        active_s=active_mask(mm,signal_dates); valid=active_s & (~pd.isna(signal_dates)); valid &= np.asarray(sig.count120>=a.MIN_LIST_DAYS); valid &= np.isfinite(sig[['liq20']].to_numpy()).all(axis=1)
        if not valid.any(): continue
        idx=np.flatnonzero(valid); rec={'signal_date':signal_dates[idx],'trade_date':exec_dates[idx],'code':code,'liq20':sig.liq20.to_numpy()[idx].astype(float)}
        ...
        for csrc,cdst in [('open','exec_open'),('high','exec_high'),('low','exec_low'),('volume','exec_volume'),('factor','exec_factor')]:
            rec[cdst]=ex[csrc].to_numpy()[idx].astype(float) if csrc in ex else np.full(len(idx),np.nan)
        fr=pd.DataFrame(rec); rows_with_missing_exec += int((~np.isfinite(fr[['exec_open','exec_high','exec_low','exec_volume']]).all(axis=1)).sum()); frames.append(fr)
    ...
    p['liq_rank_pct']=p.groupby('signal_date').liq20.rank(pct=True,method='average',ascending=False); liq_ok=p.liq_rank_pct<=a.LIQ_KEEP_PCT
    for w in a.RMOM_WINDOWS:
        col=f'rmom{w}'; out=f'rmom{w}_pct'; p[out]=np.nan; mask=liq_ok&p[col].notna(); p.loc[mask,out]=p.loc[mask].groupby('signal_date')[col].rank(pct=True,method='average',ascending=False)
    for w in a.IVOL_WINDOWS:
        col=f'ivol{w}'; out=f'ivol{w}_pct'; p[out]=np.nan; mask=liq_ok&p[col].notna(); p.loc[mask,out]=p.loc[mask].groupby('signal_date')[col].rank(pct=True,method='average',ascending=True)
    for rw in a.RMOM_WINDOWS:
        for vw in a.IVOL_WINDOWS:
            rc=f'rmom{rw}_pct'; vc=f'ivol{vw}_pct'; sc=f'score_{rw}_{vw}'; pc=sc+'_pct'; p[sc]=np.where(p[rc].notna()&p[vc].notna(),.5*(1-p[rc])+.5*(1-p[vc]),np.nan); p[pc]=np.nan; mask=p[sc].notna(); p.loc[mask,pc]=p.loc[mask].groupby('signal_date')[sc].rank(pct=True,method='average',ascending=False)
    p['core_eligible']=p[f'rmom{a.CORE_RMOM}_pct'].notna()&p[f'ivol{a.CORE_IVOL}_pct'].notna(); core=p[p.core_eligible]; gs=core.groupby('signal_date').size(); full=p.groupby('signal_date').size()
    ...
    pd.DataFrame([audit]).to_csv(OUT/'signal_pure_panel_audit.csv',index=False); print('SIGNAL PURE PANEL',audit,flush=True)
...
if __name__=='__main__': print('signal-pure panel module; import patch() before research')
