...
import run_10y_hard_executor_v2 as v2

VOLUME_SOURCE_UNIT_SHARES=100.0

def quote_available(r):
...
def row_base_allowed(r):
    if not quote_available(r): return False
    if 'exec_can_trade' in r.index and not bool(r.exec_can_trade): return False
    return True

def row_buy_allowed(r):
    if not row_base_allowed(r): return False
    if 'exec_buy_allowed' in r.index and not bool(r.exec_buy_allowed): return False
    return True

def row_sell_allowed(r):
    if not row_base_allowed(r): return False
    if 'exec_sell_allowed' in r.index and not bool(r.exec_sell_allowed): return False
    return True
...
def hard_simulate(panel,cal,members,cost_mult=1.0):
    ...
    for j,d in enumerate(dates):
        ...
        for c,pp in list(pos.items()):
            if c in g.index and np.isfinite(g.loc[c].exec_open): pp.last_price=float(g.loc[c].exec_open)
                ...
                oldp=pos.pop(c); trades.append({'variant':'hard_v3','code':c,'entry_date':oldp.entry_date,'exit_date':td,'net_pnl':-oldp.entry_cost,'net_return':-1.0,'exit_reason':'membership_end_writeoff'})
        ...
        for c in sorted(list(pos)):
            ...
            r=g.loc[c]
            if not row_sell_allowed(r): continue
            locked=abs(float(r.exec_high)-float(r.exec_low))<1e-12 and abs(float(r.exec_open)-float(r.exec_high))<1e-12
            if locked: continue
            px=float(r.exec_open)*(1-slip); gross=pos[c].units*px; cost=sim.fee(gross,'sell',td,cost_mult); oldp=pos.pop(c)
            cash+=gross-cost; turnover+=gross
            trades.append({'variant':'hard_v3','code':c,'entry_date':oldp.entry_date,'exit_date':td,'net_pnl':gross-cost-oldp.entry_cost,'net_return':(gross-cost)/oldp.entry_cost-1,'exit_reason':'rank_exit'})
            timing.append({'variant':'hard_v3','signal_date':pd.Timestamp(d),'trade_date':td,'side':'sell','code':c,'gross':gross})
        ...
        for c in target:
            ...
            rawvol=v2.raw_share_volume(float(r.exec_volume),factor); maxraw=v2.max_participation_shares(float(r.exec_volume),factor,sim.VOLUME_PARTICIPATION)
            ...
            actual=float(shares/rawvol) if np.isfinite(rawvol) and rawvol>0 else np.nan
            timing.append({'variant':'hard_v3','signal_date':pd.Timestamp(d),'trade_date':td,'side':'buy','code':c,'gross':gross,'raw_shares':shares,'raw_daily_share_volume':rawvol,'actual_participation':actual})
        if len(pos)>old.N_HOLD: raise RuntimeError('position cap violation')
        ...
        for day in seg:
            ...
            nav=cash+sum(pp.units*pp.last_price for pp in pos.values()); equity.append({'variant':'hard_v3','signal_date':pd.Timestamp(d),'trade_date':pd.Timestamp(day),'equity':nav,'cash':cash,'positions':len(pos)})
    ...
    if len(tm) and 'actual_participation' in tm:
        ...
        if len(ap) and (ap>sim.VOLUME_PARTICIPATION+1e-12).any(): raise RuntimeError('participation violation')
...
if __name__=='__main__': print({'executor':'v3','volume_source_unit_shares':100,'supports_buy_sell_flags':1})
